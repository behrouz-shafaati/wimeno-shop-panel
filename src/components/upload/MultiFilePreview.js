...
import { getFileFormat, getFileThumb } from '../../utils/getFileFormat';
...
export default function MultiFilePreview({
  ...
  console.log('files231:', files);
  const hasFile = files.length > 0;
  return (
    <>
      <List disablePadding style={{ height: '80px' }}>
        <AnimatePresence>
          {files.map((file) => {
            const { key, name, size, preview } = getFileData(file);
            const fileType = getFileFormat(file.path || file.url);
            console.log(fileType);
            const isImage = getFileFormat(file.path || file.url) === 'image';
            console.log(file);
              ...
              return (
                ...
                >
                  {isImage ? (
                    ...
                    getFileThumb(file.path, { width: 79, height: 79 })
                  )}
                  <IconButton
                    size="small"
